  ...
  return (
    ...
    >
      <div className="flex items-center justify-between">
        ...
        <div className="flex gap-2">
          <button
            ...
            disabled={loading}
            className="bg-gray-700 hover:bg-gray-600 px-4 py-2 rounded-lg text-white transition-all flex items-center gap-2"
          >
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        <div className="bg-gradient-to-br from-purple-600/20 via-purple-500/20 to-fuchsia-600/20 rounded-xl p-6 border border-purple-500/30 shadow-lg" style={{ boxShadow: '0 0 30px rgba(168, 85, 247, 0.15)' }}>
          <DollarSign className="w-8 h-8 text-purple-400 mb-3" />
          <div className="text-2xl font-bold text-white mb-1">{stats.totalRevenue.toLocaleString('cs-CZ')} Kč</div>
          <div className="text-purple-300 text-sm">Total Revenue</div>
        </div>
        <div className="bg-gradient-to-br from-purple-600/20 via-purple-500/20 to-pink-600/20 rounded-xl p-6 border border-purple-500/30 shadow-lg" style={{ boxShadow: '0 0 30px rgba(168, 85, 247, 0.15)' }}>
          <TrendingDown className="w-8 h-8 text-purple-400 mb-3" />
          <div className="text-2xl font-bold text-white mb-1">{stats.totalWithdrawals.toLocaleString('cs-CZ')} Kč</div>
          <div className="text-purple-300 text-sm">Total Withdrawals</div>
        </div>
        <div className="bg-gradient-to-br from-fuchsia-600/20 via-purple-500/20 to-purple-600/20 rounded-xl p-6 border border-purple-500/30 shadow-lg" style={{ boxShadow: '0 0 30px rgba(168, 85, 247, 0.15)' }}>
          <TrendingUp className="w-8 h-8 text-purple-400 mb-3" />
          <div className="text-2xl font-bold text-white mb-1">{netProfit.toLocaleString('cs-CZ')} Kč</div>
          <div className="text-purple-300 text-sm">Net Profit</div>
        </div>
        <div className="bg-gradient-to-br from-pink-600/20 via-purple-500/20 to-purple-600/20 rounded-xl p-6 border border-purple-500/30 shadow-lg" style={{ boxShadow: '0 0 30px rgba(168, 85, 247, 0.15)' }}>
          <RefreshCw className="w-8 h-8 text-purple-400 mb-3" />
          <div className="text-2xl font-bold text-white mb-1">{stats.pendingCount}</div>
          <div className="text-purple-300 text-sm">Pending Transactions</div>
        </div>
      </div>
            ...
            <tbody>
              {filteredTransactions.map((transaction) => (
                <tr key={transaction.id} className="border-b border-gray-700/50 hover:bg-gray-700/30">
                  <td className="py-3 px-4 text-gray-300 font-mono text-sm">{String(transaction.id).slice(0, 8)}...</td>
                  <td className="py-3 px-4 text-white">{transaction.users?.display_name || 'Unknown'}</td>
                  ...
                  <td className="py-3 px-4 text-gray-300">{transaction.payment_method || 'N/A'}</td>
                  <td className="py-3 px-4">
                    <span className={`px-3 py-1 rounded-full text-xs font-medium ${
                  ...
                  <td className="py-3 px-4">
                    <div className="flex items-center justify-end gap-2">
                      {transaction.status === 'pending' && (
                        <>
                          <button
                            ...
                            className="text-purple-400 hover:text-purple-300 text-sm px-3 py-1 rounded bg-purple-500/10 hover:bg-purple-500/20 transition-all flex items-center gap-1 border border-purple-500/30"
                          >
                            <CheckCircle size={14} />
                            Approve
                          </button>
                      ...
                      <button
                        ...
                        className="text-purple-400 hover:text-purple-300 text-sm px-3 py-1 rounded bg-purple-500/10 hover:bg-purple-500/20 transition-all border border-purple-500/30"
                      >
                        View
                      </button>
                    </div>
      ...
      {selectedTransaction && (
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 p-4" onClick={() => setSelectedTransaction(null)}>
          <div className="bg-gradient-to-br from-gray-900 to-purple-900/20 rounded-xl border border-purple-500/30 p-6 max-w-2xl w-full shadow-2xl" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
                <div className="bg-gray-800/50 rounded-lg p-4 border border-purple-500/20">
                  ...
                  <div className="text-white font-mono text-sm">{String(selectedTransaction.id)}</div>
                </div>
                <div className="bg-gray-800/50 rounded-lg p-4 border border-purple-500/20">
                  <div className="text-gray-400 text-sm mb-1">User</div>
                  <div className="text-white">{selectedTransaction.users?.display_name || 'Unknown'}</div>
                </div>
              </div>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="bg-gray-800/50 rounded-lg p-4 border border-purple-500/20">
                  ...
                  <span className={`inline-block px-3 py-1 rounded-full text-xs font-medium ${
                    selectedTransaction.type === 'deposit' ? 'bg-purple-500/20 text-purple-400' :
                ...
                <div className="bg-gray-800/50 rounded-lg p-4 border border-purple-500/20">
                  ...
                  <span className={`inline-block px-3 py-1 rounded-full text-xs font-medium ${
                    selectedTransaction.status === 'completed' ? 'bg-purple-500/20 text-purple-400' :
              ...
              {selectedTransaction.type === 'withdrawal' && selectedTransaction.withdrawal_method && (
                <div className="bg-gradient-to-br from-pink-500/10 to-fuchsia-500/10 rounded-lg p-4 border border-pink-500/30">
                  <div className="text-pink-300 text-sm mb-1 font-semibold">Withdrawal Method</div>
                  <div className="text-white font-bold text-lg">{selectedTransaction.withdrawal_method}</div>
                </div>
              )}

              {selectedTransaction.type === 'withdrawal' && selectedTransaction.withdrawal_details && (
                <div className="col-span-2 bg-gradient-to-br from-purple-500/10 to-pink-500/10 rounded-lg p-4 border border-purple-500/30">
                  <div className="text-purple-300 text-sm mb-3 font-semibold flex items-center gap-2">
                    <DollarSign size={16} />
                    Payment Details to Send Money
                  </div>
                  <div className="space-y-2 bg-gray-900/50 rounded p-3">
                    {Object.entries(selectedTransaction.withdrawal_details).map(([key, value]) => (
                      <div key={key} className="flex justify-between items-center py-1 border-b border-purple-500/20 last:border-0">
                        <span className="text-purple-300 text-sm capitalize font-medium">{key.replace(/_/g, ' ')}:</span>
                        <span className="text-white text-sm font-mono bg-purple-500/10 px-2 py-1 rounded">{String(value)}</span>
                      </div>
                    ))}
                  </div>
                </div>
              ...
              {selectedTransaction.type !== 'withdrawal' && selectedTransaction.payment_method && (
                <div className="bg-gray-800/50 rounded-lg p-4 border border-purple-500/20">
                  ...
                  <div className="text-white">{selectedTransaction.payment_method}</div>
                </div>
              )}
              ...
              {selectedTransaction.users?.email && (
                <div className="bg-gray-800/50 rounded-lg p-4 border border-purple-500/20">
                  ...
                  <div className="text-white font-mono text-sm">{selectedTransaction.users.email}</div>
                </div>
              )}

              <div className="bg-gray-800/50 rounded-lg p-4 border border-purple-500/20">
                <div className="text-gray-400 text-sm mb-1">Created At</div>
                <div className="text-white">{new Date(selectedTransaction.created_at).toLocaleString('cs-CZ')}</div>
              </div>
              ...
              {selectedTransaction.status === 'pending' && (
                <div className="flex gap-3 pt-4 border-t border-purple-500/20">
                  <button
                    ...
                    className="flex-1 bg-purple-600 hover:bg-purple-500 px-4 py-3 rounded-lg text-white font-medium transition-all shadow-lg shadow-purple-500/30 flex items-center justify-center gap-2"
                  >
                    <CheckCircle size={18} />
                    Approve Transaction
                  </button>
                  <button
                    ...
                    className="flex-1 bg-pink-600 hover:bg-pink-500 px-4 py-3 rounded-lg text-white font-medium transition-all shadow-lg shadow-pink-500/30 flex items-center justify-center gap-2"
                  >
                    <X size={18} />
                    Reject Transaction
                  </button>
