  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-6">
      <div className="flex justify-between items-center">
        <div>
          <h2 className="text-2xl font-bold bg-surface bg-clip-text text-transparent flex items-center gap-2">
            <Settings className="w-6 h-6 text-accent" />
            System Settings
          </h2>
          <p className="text-accent/80 text-sm">Configure platform settings and manage promo codes</p>
        </div>
        <button
      ...
      <div className="flex gap-2 border-b border-line">
        <button
          ...
          className={`px-6 py-3 font-medium transition-all ${
            activeTab === 'settings'
              ? 'text-accent border-b-2 border-purple-400'
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {loading ? (
            ...
            settings.map((setting) => (
              <div key={setting.id} className="bg-surface rounded-xl border border-line p-6 shadow-lg">
                <div className="flex justify-between items-start mb-2">
                  <h3 className="text-lg font-bold text-ink">{setting.key}</h3>
                  <span className={`px-2 py-1 rounded text-xs font-medium ${
                    setting.category === 'finance' ? 'bg-accent-soft text-accent' :
                ...
                <div className="bg-surface rounded px-3 py-2 text-accent font-mono text-sm mb-4 overflow-x-auto border border-line">
                  {JSON.stringify(setting.value)}
                </div>
                <button
                  onClick={() => openEditModal(setting)}
                  className="w-full bg-accent hover:opacity-90 text-on-accent px-4 py-2 rounded text-ink text-sm transition flex items-center justify-center gap-2 "
                >
                  <Edit size={16} />
                  Edit Setting
                </button>
        ...
        <div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {promoCodes.map((promo) => (
              <div key={promo.id} className={`bg-surface rounded-xl border p-6 shadow-lg ${promo.is_active ? 'border-line' : 'border-line/50'}`} style={{ boxShadow: promo.is_active ? '0 0 30px rgba(168, 85, 247, 0.1)' : 'none' }}>
                <div className="flex justify-between items-start mb-4">
                  <div className="flex items-center gap-2">
                    <Gift className="w-5 h-5 text-accent" />
                    <h3 className="text-xl font-bold text-ink font-mono">{promo.code}</h3>
                  </div>
                  <span className={`px-2 py-1 rounded text-xs font-medium ${
                    promo.is_active ? 'bg-accent-soft text-accent' : 'bg-gray-500/20 text-ink-muted'
                  }`}>
                ...
                <div className="flex gap-2">
                  <button
                    ...
                    className={`flex-1 px-3 py-2 rounded text-sm transition ${
                      promo.is_active
                        ? 'bg-gray-600 hover:bg-gray-500 text-ink'
                  ...
                  <button
                    ...
                    className="px-3 py-2 rounded text-sm bg-red-600 hover:bg-red-500 text-ink transition"
                  >
                    <Trash2 size={16} />
                  </button>
                </div>
      ...
      {editModal && (
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-surface rounded-xl border border-line p-6 max-w-2xl w-full shadow-2xl">
            ...
            <div className="mb-4">
              ...
              <textarea
                ...
                className="w-full bg-surface border border-line rounded-lg px-4 py-2 text-ink font-mono focus:outline-none focus:border-accent"
                rows={6}
              />
        ...
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-surface rounded-xl border border-line p-6 max-w-2xl w-full shadow-2xl">
            ...
            <div className="space-y-4">
              <div>
                <label className="block text-ink-muted mb-2">Promo Code *</label>
                <input
                  type="text"
                  value={newPromo.code}
                  onChange={(e) => setNewPromo({ ...newPromo, code: e.target.value.toUpperCase() })}
                  placeholder="SUMMER2025"
                  className="w-full bg-surface border border-line rounded-lg px-4 py-2 text-ink focus:outline-none focus:border-accent font-mono"
                />
              </div>

              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-ink-muted mb-2">Discount Type *</label>
                  <select
                    value={newPromo.discount_type}
                    onChange={(e) => setNewPromo({ ...newPromo, discount_type: e.target.value as 'percentage' | 'fixed' })}
                    className="w-full bg-surface border border-line rounded-lg px-4 py-2 text-ink focus:outline-none focus:border-accent"
                  >
                ...
                <div>
                  <label className="block text-ink-muted mb-2">Discount Value *</label>
                  <input
                    type="number"
              ...
              <div>
                <label className="block text-ink-muted mb-2">Max Uses</label>
                <input
                  type="number"
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-ink-muted mb-2">Valid From</label>
                  <input
                    type="date"
                ...
                <div>
                  <label className="block text-ink-muted mb-2">Valid Until</label>
                  <input
                    type="date"
