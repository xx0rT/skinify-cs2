    ...
    if (req.method === 'POST') {
      ...
      if (kycEnabled) {
        ...
        const { data: restrictions } = await supabase
          ...
          .select('*')
          .eq('user_id', user.id)
          .eq('restriction_type', 'deposit_blocked')
          .eq('is_active', true);
        ...
        const { data: thresholdSetting } = await supabase
          ...
          .select('value')
          .eq('key', 'kyc_deposit_threshold')
          .single();

        const kycThreshold = parseFloat(thresholdSetting?.value || '1000');

        // If amount exceeds threshold and user doesn't have valid KYC, block the deposit
        if (amountInEUR >= kycThreshold && !hasValidKYC) {
          // Create restriction
          await supabase
            .from('kyc_restrictions')
            .insert({
              ...
              reason: 'kyc_required',
              details: `Deposit amount (€${amountInEUR.toFixed(2)}) exceeds KYC threshold (€${kycThreshold})`,
              threshold_exceeded: amountInEUR,
              is_active: true,
            });
          ...
          await supabase
            ...
            .update({
              kyc_required: true,
              kyc_required_reason: 'deposit_threshold',
            })
            .eq('id', user.id);
          ...
          await supabase
            ...
            .insert({
              ...
              event_description: `Deposit blocked - KYC required for amount €${amountInEUR.toFixed(2)}`,
              performed_by_role: 'system',
              metadata: {
                amount_eur: amountInEUR,
                amount_czk: depositData.amount,
          ...
          return new Response(
            JSON.stringify({
              ...
              details: `Deposits over €${kycThreshold} require identity verification`,
              amount: depositData.amount,
              amountEUR: amountInEUR.toFixed(2),
              threshold: kycThreshold,
              kycRequired: true,
            }),
            {
